...
def getUserShips(board):
    print("To play the game you must first place your ships. Each player has one 2 grid ship, two 3 grid ships, one 4 grid ship, and one 5 grid ship.")
    print("To place your ships, you will enter the starting and ending coordinates of each ship when prompted. Ex: A3")
    userPrompt = [('first', 5), ('second', 3), ('third', 3), ('fourth', 4), ('fifth'), 5]
    for order, length in userPrompt:
        valid = False
        while valid is False:
            starting = getXYCords(input(f'Please enter the starting coordinate of your {place} ship with length {length}. Ex: A3'))
            ending = getXYCords(input(f'Please enter the ending coordinate of your {place} ship with length {length}. Ex: C3'))
